...
| `docker-compose.yml` 从未验证 | 待做(2026-09-14,见 §39.4) | 本机 Docker Desktop 没运行,compose 这条路径**一次都没跑过**。要在 README 里写成开箱即用就必须先实测:`docker compose up` 起 MySQL + 后端,再走一遍建库/启动/登录冒烟。README 已照实标「未验证」 |
| AI 在思维链模型下经常吐空 | 待做(2026-09-14,见 §39.4) | `AI_MODEL=qwen3.7-plus` 是思维链模型,而 `StreamReducer` 只读 `delta.content`:实测 4 次里 3 次只吐空串或只吐工具名(截图时重试 6 次才拿到真回答)。两条修法:换成非思维链模型(文档已建议),或在空回答时给用户一句明确的话而不是空气泡。这是**功能级**缺陷,不是渲染问题 |
