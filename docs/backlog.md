...
| C3 | 工具结果没有信封 | **进行中**(§24):可判定的四项已收(参数强转 ×3 收成一处 + 说清失败原因、schema 加 `required`、错误信封改由 ObjectMapper 构造、重名说清是谁)。**剩四条**见 §24.4:双重截断且可能切在 JSON 中间、`AbstractTool.json()` 把序列化失败吞成假成功、`ChatStreamListener` 仍是假 seam |
| C4 | 对话生命周期与取消不在内核里 | **进行中**(§25):收掉「线程池拒绝时客户端什么也收不到」这条(唯一必须由提交方收尾的路径)。**剩四条**见 §25.3:线程池尺寸与意图不符、轮间无存活检查(浏览器「停止」传不到服务端)、无心跳帧、断连被记成服务故障 —— 每条都要先决定一件事 |
