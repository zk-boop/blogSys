...
评审报告 C 节的七项（viewer 作为被接受的依赖、上传资产命名约定散在三处、工具结果没有
信封、对话生命周期与取消、用户显示 fallback 约 20 份、懒加载兜底横跨三处、
`fetchMe` 从未被调用）**一项都还没做**。其中前两项属于 02 与 08 的「仍未覆盖」，
后五项是独立工作项 —— 见 `docs/backlog.md` 的 v5 待办。
