...
- **文章**:发布/编辑/删除(仅作者或管理员)、草稿箱、列表分页、标签筛选、标题/内容搜索、热门文章(按浏览量,上限 20)、Markdown 渲染 + 目录 + 代码高亮、图片上传(5MB,封面裁剪 + 缩略图)
- **互动**:嵌套评论(回复挂一级评论)、点赞、收藏(个人中心「我的收藏」)
- **订阅**:`/rss` 输出 RSS 2.0(最新 20 篇),匿名口径 —— 草稿与被封禁作者不出现
...
这份仓库的默认值是**为本地开发方便**而设的。直接拿去上线,下面五条每一条都会出事:

| # | 改什么 | 不改会怎样 |
|---|---|---|
| 1 | `JWT_SECRET` 用随机值(≥32 字符,`openssl rand -base64 48`) | 任何人都能用仓库里那个公开的默认密钥**伪造任意用户的 token**。程序已经兜住这一条:非 local 启动时没给会被**直接拒绝启动** |
| 2 | 管理员口令:用 `BLOGSYS_ADMIN_PASSWORD` 指定,或从启动日志里抄走随机口令并立即改掉 | `admin / admin123` 是写在 README 里的公开口令。非 local 首次启动会随机生成并打在 WARN 日志里 |
| 3 | 数据库账号别用 `root` + 弱口令 | `application-local.yml` 里那份 `root/123456` 只是本地配置(不在版本控制里),但它常被照抄到线上 |
| 4 | 演示数据:不想要就 `mysql -uroot -p blog_sys < tools/seed/clear.sql` | `tools/seed/` 那 14 个账号口令统一 `demo1234`,**能登录、能发文章** —— 留着就等于开了 14 个发帖账号 |
| 5 | AI 助手:`AI_API_KEY` 自己申请,额度自负;**在意回答稳定性就用非思维链模型** | 不配 key 时 AI 会明确回「AI 服务未配置」,其它功能不受影响;思维链模型下正文可能为空(见 `docs/listing.md` 第八节) |

完整的部署步骤、环境变量表与常见问题见 **[`docs/deploy.md`](docs/deploy.md)** —— 那份文档里的每一步都在干净副本里实测过。
## 截图
